...
class MouvementService 
{
    ...
    {
        $mouvementRepository = $this->em->getRepository('App:Mouvement');
        $mouvementAutomatiqueRepository = $this->em->getRepository('App:MouvementAutomatique');
        
        //Récupération des mouvements automatique actifs sur le compte
        $mouvement_automatique = $mouvementAutomatiqueRepository->getMouvementAutomatiqueActifCompte($compte->getId());
        ...
        if(count($mouvement_automatique) > 0) {
            ...
            foreach ($mouvement_automatique as $mvt_auto) {
                ...
                if(date('t',mktime(0, 0, 0, date('m'), 1, date('Y'))) - date('d') <= 5) {
                    ...
                    if ($mouvementRepository->getMouvementCompte($compte->getId(), $mvt_auto['libelle'], $mvt_auto['numeroJour'], $mvt_auto['credit'], $mvt_auto['debit'], $mois_suivant, true) == false) {
                        $mouvement->setCompte($compte);
                    ...
                    if ($mouvementRepository->getMouvementCompte($compte->getId(), $mvt_auto['libelle'], $mvt_auto['numeroJour'], $mvt_auto['credit'], $mvt_auto['debit'], date('m'), false) == false) {
                        $mouvement->setCompte($compte);
                        $mouvement->setTraite(0);
                        $mouvement->setLibelle($mvt_auto['libelle']);
